...
import { Search, Plus, Upload, Eye, Edit2, Trash2, RefreshCw, ArchiveRestore } from 'lucide-react';
...
export function Catalog({ kind }: { kind: 'students' | 'books' }) {
  ...
  const [trashMode,setTrashMode]=useState(false); const [selected,setSelected]=useState<Set<number>>(new Set());
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const data = await requestJson(`/api/${kind}${trashMode?'/trash':''}?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`, { signal: controller.signal });
  ...
  }, [kind, page, search, version, trashMode]);
  useEffect(()=>setSelected(new Set()),[page,search,trashMode,version]);
  ...
  async function remove(row: Row) {
    const reason=window.prompt(`Why should ${isStudent ? row.name : row.title} be moved to Trash? It can be restored for 30 days.`,'Record no longer required');
    if (!reason) return;
    setBusy(true); setError('');
    try { await requestJson(`/api/${kind}/${row.id}`, { method: 'DELETE',...jsonBody({reason}) }); setNotice(`${singular} moved to Trash for 30 days.`); reload(); }
    catch (e) { setError((e as Error).message); } finally { setBusy(false); }
  }
  async function bulkDelete(){
    const reason=window.prompt(`Why should ${selected.size} selected ${kind} be moved to Trash for 30 days?`,'Records no longer required');if(!reason)return;
    setBusy(true);setError('');try{const data=await requestJson(`/api/${kind}/bulk-delete`,{method:'POST',...jsonBody({ids:[...selected],reason})});setNotice(`${data.count} ${kind} moved to Trash.`);reload();}catch(e){setError((e as Error).message);}finally{setBusy(false);}
  }
  async function restoreSelected(){
    if(!window.confirm(`Restore ${selected.size} selected ${kind}?`))return;
    setBusy(true);setError('');try{const data=await requestJson(`/api/${kind}/trash/restore`,{method:'POST',...jsonBody({ids:[...selected]})});setNotice(`${data.count} ${kind} restored.`);reload();}catch(e){setError((e as Error).message);}finally{setBusy(false);}
  }
  useEffect(() => {
  ...
  return <div className="p-4 md:p-6 space-y-5">
    <div className="flex flex-wrap justify-between items-center gap-3"><div><h1 className="text-xl font-semibold text-slate-900">{trashMode?`${singular} Trash`:isStudent ? 'Students' : 'Books Catalog'}</h1><p className="text-sm text-slate-500 mt-1">{trashMode?'Deleted records can be restored for 30 days.':isStudent ? 'University student records and CSV imports' : 'Book records, copy counts, availability, and borrowing history'}</p></div><div className="flex flex-wrap gap-2"><button onClick={()=>{setTrashMode(v=>!v);setPage(1);}} className="flex gap-2 items-center border px-4 py-2 rounded-lg text-sm"><ArchiveRestore size={16}/>{trashMode?'Back to active records':'Open Trash'}</button>{!trashMode&&<><button onClick={() => setImporting(true)} className="flex gap-2 items-center border px-4 py-2 rounded-lg text-sm"><Upload size={16} />Import CSV</button><button disabled={!layout || !!layoutError} onClick={() => openForm()} className="flex gap-2 items-center bg-blue-800 text-white px-4 py-2 rounded-lg text-sm"><Plus size={16} />Add {singular}</button></>}</div></div>
    {!isStudent && !trashMode && <div className="grid grid-cols-2 md:grid-cols-4 gap-3">{[['total', 'Total copies'], ['available', 'Available'], ['issued', 'Issued copies'], ['overdue', 'Overdue copies']].map(([key, label]) => <div key={key} className="bg-white border rounded-xl p-4"><p className="text-sm text-slate-500">{label}</p><p className="font-bold text-2xl text-blue-900 mt-1">{loading || error ? '—' : stats[key] ?? 0}</p></div>)}</div>}
    ...
    {selected.size>0&&<div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex flex-wrap items-center justify-between gap-3"><span className="text-sm text-blue-900">{selected.size} selected</span><button disabled={busy} onClick={trashMode?restoreSelected:bulkDelete} className={`px-4 py-2 rounded-lg text-sm text-white disabled:opacity-40 ${trashMode?'bg-emerald-700':'bg-red-700'}`}>{trashMode?'Restore selected':'Move selected to Trash'}</button></div>}
    <div className="bg-white border rounded-xl overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-slate-50"><tr><th className="p-3"><input aria-label="Select page" type="checkbox" checked={!!rows.length&&rows.every(r=>selected.has(r.id))} onChange={e=>setSelected(e.target.checked?new Set(rows.map(r=>r.id)):new Set())}/></th><th className="p-3 text-left whitespace-nowrap">Sr #</th>{columns.map(f => <th key={f.key} className="p-3 text-left whitespace-nowrap">{f.label}</th>)}{!isStudent && !trashMode && <th className="p-3 text-left">Availability</th>}{trashMode&&<th className="p-3 text-left">Expires</th>}<th className="p-3 text-left">Actions</th></tr></thead><tbody>
      {loading ? <tr><td className="p-8 text-center text-slate-500" colSpan={columns.length + 4}>Loading records…</td></tr> : error ? <tr><td className="p-8 text-center" colSpan={columns.length + 4}>Records could not be loaded.</td></tr> : !rows.length ? <tr><td className="p-8 text-center text-slate-500" colSpan={columns.length + 4}>{search ? 'No matching records.' : trashMode?'Trash is empty.':`No ${kind} yet. Add one or import a CSV.`}</td></tr> : rows.map((row, i) => <tr key={row.id} className="border-t hover:bg-slate-50"><td className="p-3"><input aria-label={`Select ${isStudent?row.name:row.title}`} type="checkbox" checked={selected.has(row.id)} onChange={e=>setSelected(current=>{const next=new Set(current);e.target.checked?next.add(row.id):next.delete(row.id);return next;})}/></td><td className="p-3 text-slate-500">{(page - 1) * limit + i + 1}</td>{columns.map(f => <td key={f.key} title={String(row[f.key] ?? '')} className={`p-3 max-w-60 truncate ${f.key === 'name' || f.key === 'title' ? 'font-medium' : 'text-slate-600'}`}>{row[f.key] === '' || row[f.key] == null ? '—' : row[f.key]}</td>)}{!isStudent && !trashMode && <td className="p-3 whitespace-nowrap"><span className={`px-2 py-1 rounded-full text-xs ${row.overdueCount ? 'bg-red-50 text-red-700' : Number(row.availableCount)===0 ? 'bg-amber-50 text-amber-800' : 'bg-emerald-50 text-emerald-800'}`}>{row.availableCount} / {row.total_copies} available</span>{Number(row.overdueCount)>0&&<p className="text-red-700 text-xs mt-1">{row.overdueCount} overdue</p>}</td>}{trashMode&&<td className="p-3 text-xs whitespace-nowrap">{row.purge_at||'—'}</td>}<td className="p-3"><div className="flex gap-2"><button aria-label={`View ${isStudent ? row.name : row.title}`} onClick={() => setViewing(row)} className="p-1.5 hover:bg-blue-50 rounded"><Eye size={16} /></button>{!trashMode&&<><button aria-label={`Edit ${isStudent ? row.name : row.title}`} onClick={() => openForm(row)} className="p-1.5 hover:bg-blue-50 rounded"><Edit2 size={16} /></button><button disabled={busy} aria-label={`Delete ${isStudent ? row.name : row.title}`} onClick={() => remove(row)} className="p-1.5 text-red-600 hover:bg-red-50 rounded disabled:opacity-40"><Trash2 size={16} /></button></>}</div></td></tr>)}
