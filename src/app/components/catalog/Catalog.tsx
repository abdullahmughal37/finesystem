...
export function Catalog({ kind }: { kind: 'students' | 'books' }) {
  ...
  function openForm(row?: Row) {
    ...
    setEditing({...(row ? { ...row } : Object.fromEntries(fields.map(f => [f.key, f.key === 'status' ? 'Active' : f.core && f.type === 'number' ? 0 : '']))),layoutRevision:layout?.revision}); setFormError('');
  }
  ...
  return <div className="p-4 md:p-6 space-y-5">
    ...
    {editing && <Modal title={`${editing.id ? 'Edit' : 'Add'} ${singular}`} onClose={() => setEditing(null)} busy={busy}><form onSubmit={submit}><div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">{formFields.map(f => <label key={f.key} className={`block text-sm font-medium text-slate-700 ${f.width === 'full' ? 'sm:col-span-2' : ''}`}>{f.label}{f.required ? ' *' : ''}{f.type === 'select' ? <select aria-label={`${f.label}${f.required ? ' *' : ''}`} required={f.required} value={editing[f.key] ?? ''} onChange={e => setEditing({ ...editing, [f.key]: e.target.value })} className="block w-full mt-1 border p-2 rounded-lg"><option value="">Select {f.label}</option>{f.options.map(s => <option key={s}>{s}</option>)}</select> : f.type === 'textarea' ? <textarea required={f.required} value={editing[f.key] ?? ''} maxLength={5000} rows={3} onChange={e => setEditing({ ...editing, [f.key]: e.target.value })} className="block w-full mt-1 border p-2 rounded-lg" /> : <input required={f.required} type={f.type || 'text'} maxLength={255} min={f.core && f.type === 'number' ? 0 : undefined} max={f.key === 'cost' ? 99999999.99 : f.key === 'pages' ? 2147483647 : undefined} step={f.key === 'cost' ? '0.01' : f.type === 'number' ? (f.core ? '1' : 'any') : undefined} value={editing[f.key] ?? ''} onChange={e => setEditing({ ...editing, [f.key]: e.target.value })} className="block w-full mt-1 border p-2 rounded-lg" />}</label>)}{isStudent && <p className="text-xs text-slate-500 sm:col-span-2">Sr # is generated for the list. Registration numbers identify student records; students may share a name. Contact numbers and email addresses are stored separately.</p>}{formError && <p role="alert" className="text-sm text-red-700 bg-red-50 p-3 rounded sm:col-span-2">{formError}</p>}</div><div className="flex justify-end gap-3 p-5 border-t"><button type="button" disabled={busy} onClick={() => setEditing(null)} className="px-4 py-2 border rounded-lg">Cancel</button><button disabled={busy} className="px-4 py-2 bg-blue-800 text-white rounded-lg disabled:opacity-50">{busy ? 'Saving…' : `Save ${singular}`}</button></div></form></Modal>}
