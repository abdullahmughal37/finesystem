  ...
  useEffect(() => {
    Promise.all([requestJson('/api/reminders/overdue'), requestJson('/api/reminders/status')])
      .then(([rows, status]) => {
        ...
        setSmtpConfigured(Boolean(status?.configured));
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button onClick={sendReminders} disabled={sending || list.length === 0 || !smtpConfigured} className="px-4 py-2 rounded-lg text-sm text-white font-medium bg-blue-600 hover:bg-blue-700 disabled:opacity-50">{sending ? "Sending..." : "Send Reminder Emails"}</button>
      </div>
      {!loading && smtpMessage && <div role="status" className={`p-3 rounded-lg text-sm ${smtpConfigured ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-800'}`}>{smtpMessage}</div>}
