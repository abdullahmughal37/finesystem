...
export function ReminderEmails() {
  ...
  useEffect(() => {
    requestJson('/api/reminders/overdue').then((rows) => setList(Array.isArray(rows) ? rows : [])).catch((error) => { setList([]); setError((error as Error).message); }).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button onClick={sendReminders} disabled={sending || list.length === 0} className="px-4 py-2 rounded-lg text-sm text-white font-medium bg-blue-600 hover:bg-blue-700 disabled:opacity-50">{sending ? "Sending..." : "Send Reminder Emails"}</button>
      </div>
