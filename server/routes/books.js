...
const db = require('../db');
const { save, remove, moveToTrash, restoreFromTrash, pageOptions } = require('../lib/catalog');
...
router.get('/', async (req, res) => {
  try {
    ...
    const where = search ? `WHERE b.deleted_at IS NULL AND (${bookSearchClause('b')})` : 'WHERE b.deleted_at IS NULL';
    ...
    const [stats] = await db.promise().query(`SELECT COALESCE(SUM(total_copies),0) total, (SELECT COUNT(*) FROM issues i JOIN books ib ON ib.id=i.book_id WHERE i.returned=0 AND ib.deleted_at IS NULL) issued, (SELECT COUNT(*) FROM issues i JOIN books ib ON ib.id=i.book_id WHERE i.returned=0 AND i.due_date<? AND ib.deleted_at IS NULL) overdue FROM books WHERE deleted_at IS NULL`, [today()]);
...
router.get('/trash',async(req,res)=>{try{await purgeExpiredCatalog(db.promise());const {page,limit,offset,search}=pageOptions(req.query);const where=search?`b.deleted_at IS NOT NULL AND b.purged_at IS NULL AND (${bookSearchClause('b')})`:'b.deleted_at IS NOT NULL AND b.purged_at IS NULL';const args=search?bookSearchArgs(search):[];const [count]=await db.promise().query(`SELECT COUNT(*) total FROM books b WHERE ${where}`,args);const [rows]=await db.promise().query(`SELECT b.* FROM books b WHERE ${where} ORDER BY b.deleted_at DESC LIMIT ? OFFSET ?`,[...args,limit,offset]);res.json({rows:rows.map(({catalog_identity,...row})=>row),total:Number(count[0].total),page,limit});}catch(e){sendError(res,e);}});
router.post('/bulk-delete',async(req,res)=>{try{const count=await moveToTrash('books',req.body?.ids,req.user,req.body?.reason);res.json({success:true,count});}catch(e){sendError(res,e);}});
router.post('/trash/restore',async(req,res)=>{try{const count=await restoreFromTrash('books',req.body?.ids);res.json({success:true,count});}catch(e){sendError(res,e);}});
router.put('/:id', async (req, res) => {
  try { await save('books',req.body,Number(req.params.id));res.json({success:true}); } catch(error) {sendError(res,error);}
});
router.delete('/:id', async (req, res) => {
  try { await remove('books',Number(req.params.id),req.user,req.body?.reason);res.json({success:true}); } catch(error) {sendError(res,error);}
});
