...
function sendError(res, error) {
  ...
  if (error.code === 'ER_DUP_ENTRY' && String(error.message).includes('uq_books_catalog_identity')) return res.status(409).json({ error: 'A matching book edition already exists. Increase Total Copies on the existing book.', code: 'duplicate' });
